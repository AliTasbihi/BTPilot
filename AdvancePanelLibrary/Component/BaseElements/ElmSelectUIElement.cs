...
using AdvancePanelLibrary.Utility.EditDesktopElement;
using AdvancePanelLibrary.Utility.EditImageCollection;
...
namespace AdvancePanelLibrary.Component.BaseElements
{
    public class ElmSelectUIElement : BasicElement
    {
        ...
        private void editElementToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var arrow = buildingBlock.InputArrowByElementTitle("Source Element");
                AutomationElement srcElm = (arrow != null) ? arrow.TransferData as AutomationElement : null;

                frm.InitForm(advancePanel.AllBuildingBlock, srcElm, SelectElementStoreable, null);
            ...
            using (var frmSelect = new SelectUIElemetForm())
            {
                ...
                if (frmSelect.GetResultForm() == true)
                {
                    ...
                    using (var frm = new EditDesktopElementForm())
                    {
                        ...
                        frm.InitForm(advancePanel.AllBuildingBlock, srcElm, SelectElementStoreable, elm);
                        var resFrom = frm.ShowDialog();
                        if (resFrom == DialogResult.OK)
                        {
                            SelectElementStoreable = frm.GetResultForm();
