...
namespace AdvancePanelLibrary.Component.BaseElements
{
    public class BasicBuildingBlock : BasicLoadClickDoubleClickAction
    {
        ...
        [Category("NoneSave")]
        public List<object> ClickableComponents { get; set; } = new List<object>();
        ...
        private string GetNameNotFoundErrorOccurred
        {
            get
            {
                return FindNameOfLabelByTitle(new string[] { "not found", "error occurred" });
            }
        }
        ...
        private string FindNameOfLabelByTitle(string[] titles)
        {
            foreach (BasicElement child in Children)
            {
                if (child is ElmLabel lbl)
                {
                    foreach (string t in titles)
                    {
                        if (lbl.Title.Contains(t, StringComparison.CurrentCultureIgnoreCase))
                        {
                            var name = child.Name;
                            return name;
                        }
        ...
        public bool IsExistElementByName(string elementName)
        {
            foreach (BasicElement element in Children)
            {
                if (String.Equals(element.Name, elementName, StringComparison.OrdinalIgnoreCase))
        ...
        public ElmArrowButton InputArrowByElementTitle(string elementTitle)
        {
            var elementName = FindNameOfLabelByTitle(new string[] { elementTitle });
            return InputArrowByElementName(elementName);
        }
        ...
        public void DrawAllElementes(Graphics graphics, Transform transform, bool isActiveBuildingBlock)
        {
            ...
            {
                GraphicFunction.DrawFillRectangle(bmpGfx, new Rectangle(0, 0, bmp.Width, bmp.Height), GraphicConstant.colorBackgroundBuildingBlock, GraphicConstant.colorActiveBuildingBlock, 2 * borderWidth);
            }
            else
            {
                GraphicFunction.DrawFillRectangle(bmpGfx, new Rectangle(0, 0, bmp.Width, bmp.Height), GraphicConstant.colorBackgroundBuildingBlock, borderColor, 2 * borderWidth);
            }
            ...
            if (isActiveBuildingBlock)
            {
                GraphicFunction.DrawLine(bmpGfx, 0, y, bmp.Width, y, GraphicConstant.colorActiveBuildingBlock, 2 * borderWidth);
            }
            else
            {
            ...
            var bmpResize = GraphicFunction.ResizeImage(bmp, Width, Height, _transform.ModelToDisplayValue(Width), _transform.ModelToDisplayValue(Height));
        ...
        public string? GetAllScriptClickableArea(int spaceNo)
        {
            ...
            foreach (var elem in ClickableComponents)
            {
                if (elem is BasicElement basicElement)
                {
                    ...
                    sb.AppendLine($"{space} {c}-{ty}:[{basicElement.BackgroundArea.Left},{basicElement.BackgroundArea.Top},{basicElement.BackgroundArea.Right},{basicElement.BackgroundArea.Bottom}]");
                }
        ...
        private void GetDetailOfBasicElement(StringBuilder sb, BasicElement basicElement, string space)
        {
            ...
            sb.AppendLine($"{space} -Area:[{basicElement.BackgroundArea.Left},{basicElement.BackgroundArea.Top},{basicElement.BackgroundArea.Right},{basicElement.BackgroundArea.Bottom}]");
        ...
        public bool IsSuccessfullyStatusOfExecution()
        {
            if (StatusOfExecution == StatusOfExecutionEnum.Finish || StatusOfExecution == StatusOfExecutionEnum.FinishLoopCompleted)
            {
                return true;
            }
            return false;
        ...
        {
            if (StatusOfExecution == StatusOfExecutionEnum.Finish || StatusOfExecution == StatusOfExecutionEnum.RunningLoop)
            {
            ...
            else if (StatusOfExecution == StatusOfExecutionEnum.FinishLoopCompleted)
            {
                var elmName = FindNameOfLabelByTitle(new string[] { "Completed" });
                var elm = ElementByName($"{elmName}ConnectorLinkOut");
                if (elm == null) return null;
                if (elm is ElmConnector cntr)
                {
                    var arrows = advancePanel.GetArrowsWithStart(cntr);
            ...
            {
                var elm = ElementByName($"{GetNameNotFoundErrorOccurred}ConnectorLinkOut");
                if (elm == null) return null;
                if (elm is ElmConnector cntr)
                {
                    var arrows = advancePanel.GetArrowsWithStart(cntr);
        ...
        // یعنی هیج کانکتور وردی و خروجی رو هدر ندارند
        public List<BasicBuildingBlock> GetIndependentBuildingBlock(BasicBuildingBlock blockBld)
        {
            var liBld = new List<BasicBuildingBlock>();
            List<ElmArrowButton> arrows = advancePanel.GetAllInputArrowsBuildingBlock(blockBld);
            foreach (var arrow in arrows)
            {
                var bld = arrow.ConnectorStart.buildingBlock;
                var conIn = bld.IsExistElementByName("HeaderConnectorLinkIn");
                var conOut = bld.IsExistElementByName("HeaderConnectorLinkOut");
                if (conIn == false && conOut == false)
                {
                    liBld.Add(bld);
                    var newBld = GetIndependentBuildingBlock(bld);
                    if (newBld.Count > 0)
                        liBld.AddRange(newBld);
                }
            }
            return liBld;
        }
        ...
        public string GetConnectorPropertyEditBox(string elementNameForArrow, string elementNameForData)
        {
            ...
            {
                return arrow.TransferData.ToString();
            }
        }
        ...
        public SelectElementStoreable GetConnectorPropertySelectElementCondition(string elementNameForArrow, string elementNameForData)
        {
            var arrow = InputArrowByElementName(elementNameForArrow);
            if (arrow == null)
            {
        ...
        public object GetConnectorPropertyFoundElement(AutomationElement[] foundElements, int currentIndex)
        {
        ...
        public object GetConnectorPropertyPositionFoundX(AutomationElement[] foundElements, int index)
        {
            if (foundElements == null || foundElements.Length == 0)
                return null;
            if (foundElements[index].GetType().HasProperty("BoundingRectangle"))
            {
                return foundElements[index].BoundingRectangle.Left;
            }
            return null;
        }
        ...
        #region General Method in Building Block
        public AutomationElement[] GetElementsWithConditionTimeout(GlobalVariablePlayer globalVariablePlayer, AutomationElement sourceElement, SelectElementStoreable selectElementStoreable, TimeSpan timeOut)
        {
            try
            {
                if (selectElementStoreable != null)
                {
                    ...
                    AutomationElement[] targetElements = conditionElement.GetTargetElements(globalVariablePlayer, sourceElement, timeOut);
