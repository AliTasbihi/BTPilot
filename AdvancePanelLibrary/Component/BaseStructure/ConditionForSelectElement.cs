...
using static AdvancePanelLibrary.Component.BaseStructure.ConditionForSelectElement.OneCondition;
...
namespace AdvancePanelLibrary.Component.BaseStructure
{
    public class ConditionForSelectElement
    {
        ...
        public AutomationElement[] GetTargetElements(GlobalVariablePlayer globalVariablePlayer, AutomationElement sourceElement, TimeSpan timer)
        {
            try
            {
                ...
                do
                {
                    AutomationElement parentElement = (sourceElement != null) ? sourceElement : globalVariablePlayer.CurrentMainWindow;
                    if (parentElement==null || !parentElement.IsAvailable)
                        throw new Exception("المان مورد نظر در حافظه وجود ندارد");

                    AutomationElement[]? elementSearchList = null;
                    AutomationElement[] elems = null;
                    for (int levelIndex = 0; levelIndex < LevelContions.Count; levelIndex++)
                    {
                        var level = LevelContions[levelIndex];
                        if (!level.GetChildrenTypeAtFirstItem(out var childrenType, out var childNo))
                            return null;
                        ...
                        if (levelIndex == 0)
                        {
                            ...
                            {
                                elementSearchList = parentElement.FindAllChildren();
                            }
                            else if (childrenType == OneCondition.ChildrenType.Descended || childrenType == OneCondition.ChildrenType.DescendedNo)
                                elementSearchList = parentElement.FindAllDescendants();
                            elems = level.GetElementInChildWithCondition(elementSearchList, childrenType, childNo);
                        }
                        else
                        {
                            var  elems0 =new List<AutomationElement>(); 
                            foreach (var elm in elementSearchList)
                            {
                                AutomationElement[]? elementSearchList0 = null;
                                if (childrenType == OneCondition.ChildrenType.Child || childrenType == OneCondition.ChildrenType.ChildNo)
                                {
                                    elementSearchList0 = elm.FindAllChildren();
                                }
                                else if (childrenType == OneCondition.ChildrenType.Descended || childrenType == OneCondition.ChildrenType.DescendedNo)
                                    elementSearchList0 = elm.FindAllDescendants();
                                var elems1 = level.GetElementInChildWithCondition(elementSearchList0, childrenType, childNo);
                                elems0.AddRange(elems1);
                            }
                            elems = elems0.ToArray();
                        }
                        
                        if (elems is null || elems.Length ==0|| LevelContions.Count == 0)
                            break;

                        AutomationElement[] elemsReturn = null;
                        if (childrenType == OneCondition.ChildrenType.ChildNo || childrenType == OneCondition.ChildrenType.DescendedNo)
                        {
                            elemsReturn = new AutomationElement[] { elems[childNo] };
                        }
                        else
                            elemsReturn = elems;
                        ...
                        if (LevelContions.Count > levelIndex + 1)
                        {
                            if (LevelContions[levelIndex + 1].conditions.Count <= 1)
                                return elemsReturn;
                        }
                        elementSearchList = elemsReturn;
                    }
                    Thread.Sleep(500);
        ...
        public class LevelContion
        {
            ...
            public AutomationElement[] GetElementInChildWithCondition(AutomationElement[] elementSearchList, OneCondition.ChildrenType childrenType, int childNo)
            {
                /*AutomationElement[] elementSearchList = null;
                if (childrenType == OneCondition.ChildrenType.Child || childrenType == OneCondition.ChildrenType.ChildNo)
                {
                    ...
                    elementSearchList = parentElement.FindAllDescendants();
                */
                if (elementSearchList is null)
                    return null;
                for (int i = 1; i < conditions.Count; i++)
                {
                    elementSearchList = GetChildrenWithCondtion(elementSearchList, conditions[i]);
                }

                if (childrenType == OneCondition.ChildrenType.ChildNo && childNo > 0 && elementSearchList is not null)
                {
                    if (childNo <= elementSearchList.Length)
                        return new AutomationElement[] { elementSearchList[childNo - 1] };
                }
            ...
            public bool GetChildrenTypeAtFirstItem(out OneCondition.ChildrenType outChildrenType, out int outChildNo)
            {
            ...
            public int DrawOneConditionToPanel(GroupBox panel, OneCondition condition, int yStartComponent)
            {
                ...
                switch (conditionType)
                {
                    case ConditionType.Children:
                        ...
                        _numericUpDownChildrenNo.Value = ChildrenNo + 1;
